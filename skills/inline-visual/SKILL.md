...
每轮在组织最终回复前静默判断一次，用户不需要点名或手动选择本技能。只有可视化能明显减少解释成本时才生成；不得为了填满正文空白而生成装饰图。
...
不要把普通卡片列表命名为“流程图”。流程、调用链、依赖图必须让用户第一眼看到节点之间的方向、分支或汇合；若没有边，就应改名为清单或对比，而不是冒充流程图。
...
- 流程/关系图必须传 `kind: "flow"` 或 `kind: "relation"`，并优先使用宿主结构类：
  - 容器：`.viz-flow`；横向流程追加 `.viz-flow--horizontal`
  - 节点：`.viz-node`（可带 `data-priority="P0|P1"`、`data-state="success"`）
  - 连线：`.viz-connector`，分支容器：`.viz-branches`，单个分支：`.viz-branch`
  - 复杂拓扑可用 SVG `path/line`；禁止仅在说明文字里写 `A → B` 代替真正连线
...
- 宽而浅优先于又高又挤
- 颜色编码要配合标签/形状，不单靠颜色
- 流程节点默认只放“短标题 + 一句结果”；代码位置、证据和长解释放进 `<details>`，不要把整段诊断文字铺在节点里
- 因果链使用单向连线；并行影响使用分支；多个分支共同决定结果时必须画出汇合节点
- P0/P1 不只换圆点颜色：同时使用 `data-priority` 的边框/侧边条，并在节点标题保留文字标签
